...
def infer_atadd_t2_type(filename, row=None):
    """
    Track2 type inference.
    Priority:
    1. label csv has a type-like column
    2. filename or relative path contains speech/sound/singing/music
    """
    if row is not None:
        for key in ["type", "audio_type", "category", "class", "domain"]:
            if key in row and row[key] is not None:
                v = str(row[key]).strip().lower()
                if v in TYPE2ID:
                    return v

    text = str(filename).lower().replace("\\", "/")

    # robust keyword matching
    if "singing" in text or "/sing/" in text or "_sing_" in text:
        return "singing"
    if "speech" in text or "/sp/" in text or "_sp_" in text:
        return "speech"
    if "sound" in text or "audio" in text or "environment" in text:
        return "sound"
    if "music" in text or "instrument" in text or "song" in text:
        return "music"
...
class atadd_dataset(Dataset):
    def __init__(
        ...
        with open(self.path_to_protocol, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                if self.return_type:
                    audio_type = infer_atadd_t2_type(filename, row)
                    if audio_type is None:
                        raise ValueError(
                            f"Cannot infer Track2 audio type for file: {filename}. "
                            f"Your label csv must contain one of columns "
                            f"type/audio_type/category/class/domain, or filename/path must contain "
                            f"speech/sound/singing/music."
                        )
                    self.file_types.append(TYPE2ID[audio_type])
    ...
    def __getitem__(self, idx):
        ...
        if self.return_type:
            type_id = self.file_types[idx]
            return waveform, filename, label, type_id
